...
```  
 ...
 Lambdas are used in method-based [!INCLUDE[vbteclinq](../../../csharp/getting-started/includes/vbteclinq_md.md)] queries as arguments to standard query operator methods such as <xref:System.Linq.Enumerable.Where*>.  
  
 When you use method-based syntax to call the <xref:System.Linq.Enumerable.Where*> method in the <xref:System.Linq.Enumerable> class (as you do in [!INCLUDE[vbteclinq](../../../csharp/getting-started/includes/vbteclinq_md.md)] to Objects and [!INCLUDE[sqltecxlinq](../../../csharp/programming-guide/concepts/linq/includes/sqltecxlinq_md.md)]) the parameter is a delegate type <xref:System.Func`2?displayProperty=fullName>. A lambda expression is the most convenient way to create that delegate. When you call the same method in, for example, the <xref:System.Linq.Queryable?displayProperty=fullName> class (as you do in [!INCLUDE[vbtecdlinq](../../../csharp/includes/vbtecdlinq_md.md)]) then the parameter type is an <xref:System.Linq.Expressions.Expression?displayProperty=fullName><Func\> where Func is any of the Func delegates with up to sixteen input parameters. Again, a lambda expression is just a very concise way to construct that expression tree. The lambdas allow the `Where` calls to look similar although in fact the type of object created from the lambda is different.  
...
## Lambdas with the Standard Query Operators  
 Many Standard query operators have an input parameter whose type is one of the <xref:System.Func`2> family of generic delegates. These delegates use type parameters to define the number and types of input parameters, and the return type of the delegate. `Func` delegates are very useful for encapsulating user-defined expressions that are applied to each element in a set of source data. For example, consider the following delegate type:  
